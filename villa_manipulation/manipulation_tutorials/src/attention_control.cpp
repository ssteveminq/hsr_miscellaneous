...
class gazepoint_fixing_map {
 public:
  gazepoint_fixing_map() :
    is_activating_(false),
    tf_buffer_(),
    listener_(tf_buffer_) {
    ros::NodeHandle nh("gazed_point_fixing_node_map");
    ...
    joint_state_sub_ = nh.subscribe("/hsrb/joint_states", 1,
                                    &gazepoint_fixing_map::JointStateCallback_, this);
    target_point_sub_ = nh.subscribe("target_point",
                                     1,
                                     &gazepoint_fixing_map::TargetPointCallback_, this);
    activation_sub_ = nh.subscribe("activate",
                                   1,
                                   &gazepoint_fixing_map::ActivationCallback_, this);
